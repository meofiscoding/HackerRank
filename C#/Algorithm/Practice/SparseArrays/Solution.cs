...
class Result
{
    ...
    {
        Dictionary<string,int> stringListDictionary = stringList.GroupBy(x => x).ToDictionary(x => x.Key, x => x.Count());
        List<int> result = new List<int>();
        foreach(string query in queries)
        {
            if(stringListDictionary.ContainsKey(query))
            {
                result.Add(stringListDictionary[query]);
            }
            else
            {
                result.Add(0);
